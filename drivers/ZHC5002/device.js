...
class ZHC5002 extends ZwaveDevice {
	async onMeshInit() {
		...
		*/
		...
		if (!(commandClassScene instanceof Error)) {
			this.registerReportListener('CENTRAL_SCENE', 'CENTRAL_SCENE_NOTIFICATION', (rawReport, parsedReport) => {
				if (rawReport &&
					...
					if (rawReport['Sequence Number'] !== PreviousSequenceNo) {
						...
						// const actualButton = reportButton; // ZHC5002 does not have- (this.getSetting('scene_offset') - 1);

						const remoteValue = {
							scene_number: reportButton.toString(),
							button: reportButton.toString(),
						...
						triggerZHC5002_scene.trigger(this, triggerZHC5002_scene.getArgumentValues, remoteValue);
			...
			let triggerZHC5002_scene = new Homey.FlowCardTriggerDevice('ZHC5002_scene');
			triggerZHC5002_scene
				.register()
		...
		let ZHC5002_setLEDlevel_run_listener = async(args) => {
		...
		let actionZHC5002_setLEDlevel = new Homey.FlowCardAction('ZHC5002_set_led_level');
		actionZHC5002_setLEDlevel
			.register()
			.registerRunListener(ZHC5002_setLEDlevel_run_listener);

		// define FlowCardAction to start the flashing mode of a LED indicator
		let ZHC5002_setLEDflash_run_listener = async(args) => {
		...
		let actionZHC5002_setLEDflash = new Homey.FlowCardAction('ZHC5002_set_led_flash');
		actionZHC5002_setLEDflash
			.register()
			.registerRunListener(ZHC5002_setLEDflash_run_listener);

		// define FlowCardAction to stop the flashing of the LED indicator
		let ZHC5002_stopLEDflash_run_listener = async(args) => {
			this.log('FlowCardAction Set LED flash: ', args.led, 'to level: ', args.level, 'on_off_period: ', args.on_off_period, 'on_off_cycles: ', args.on_off_cycles);
			let result = await args.device.node.CommandClass.COMMAND_CLASS_INDICATOR.INDICATOR_SET({
				'Indicator 0 Value': 'off/disable',
				Properties1: {
		...
		let actionZHC5002_stopLEDflash = new Homey.FlowCardAction('ZHC5002_stop_led_flash');
		actionZHC5002_stopLEDflash
			.register()
			.registerRunListener(ZHC5002_stopLEDflash_run_listener);
